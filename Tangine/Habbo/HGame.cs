...
namespace Tangine.Habbo
{
    public class HGame : ShockwaveFlash
    {
        ...
        public int RenameRegisters()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                foreach (ASMethodBody body in abc.MethodBodies)
                {
                    ...
                    if (code.FindOperationIndex(Operation.Debug) == -1) continue; // Nothing to modify.
                    if (code.FindOperationIndex(Operation.LookUpSwitch) == -1) continue; // Switch statements not supported in jump fixing process.

                    var debugInstructs = new SortedList<byte, DebugIns>(code.Instructions
                        .Where(ins => ins.OP == Operation.Debug)
                        .Cast<DebugIns>()
                        .ToDictionary(debugIns => debugIns.RegisterIndex));

                    List<ASParameter> parameters = body.Method.Parameters;
                    for (int j = 0, paramId = 1, locId = 1; j < debugInstructs.Count; j++)
                    {
                        ASParameter parameter = null;
                        string regName = string.Empty;
                        DebugIns debugIns = debugInstructs.Values[j];
        ...
        public bool DisableHandshake()
        {
            ...
            ASCode sendCode = DisableEncryption(sendMethod);
            if (sendCode != null)
            {
        ...
        public bool InjectKeySharer(bool disableCrypto)
        {
            ABCFile abc = GetABCFile(2);
            int sendMessageQNameIndex = 0;
            #region Adding Method: sendMessage(header:int, values:Array) : Boolean
            ASInstance socketConnection = abc.GetClasses("SocketConnection")[0].Instance;
            ASMethod sendMethod = socketConnection.GetMethod(1, "Boolean", "send");
            if (sendMethod == null) return false;

            ASCode sendCode = DisableEncryption(sendMethod);
            if (sendCode != null && disableCrypto)
            {
                sendMethod.Body.Code = sendCode.ToArray();
            }
            else return false;
            ...
            var headerParam = new ASParameter(abc);
            headerParam.NameIndex = abc.Pool.AddString("header");
            headerParam.TypeIndex = abc.Pool.GetMultinameIndices("int").First();
            sendMessageMethod.Parameters.Add(headerParam);

            var valuesParam = new ASParameter(abc);
            valuesParam.NameIndex = abc.Pool.AddString("values");
            valuesParam.TypeIndex = abc.Pool.GetMultinameIndices("Array").First();
            sendMessageMethod.Parameters.Add(valuesParam);

            // The method body that houses the instructions.
            var sendMessageBody = new ASMethodBody(abc);
            int sendMessageBodyIndex = abc.AddMethodBody(sendMessageBody);
            sendMessageBody.MethodIndex = sendMessageMethodIndex;
            sendMessageBody.Code = TrimSendCode(abc, sendCode);
            ...
            // The name/identifier for this method/trait.
            var qName = new QName(ConstantType.QName, abc.Pool);
            qName.NameIndex = abc.Pool.AddString("sendMessage");
            qName.NamespaceIndex = 1; // Public

            var multiname = new ASMultiname(abc.Pool);
            multiname.Data = qName;
            sendMessageQNameIndex = abc.Pool.AddMultiname(multiname);

            var methodTrait = new ASTrait(abc);
            methodTrait.QNameIndex = sendMessageQNameIndex;
            methodTrait.Data = new MethodGetterSetterTrait(TraitKind.Method, methodTrait, abc, sendMessageMethodIndex, 0);

            // I guess that's it.
            socketConnection.Traits.Add(methodTrait);
            #endregion

            ASInstance habboCommunicationDemo =
                abc.GetClasses("HabboCommunicationDemo")[0].Instance;
            ...
            List<Instruction> instructions = pubKeyVerCode.Instructions;

            int coereceCount = 0;
            foreach (Instruction instruct in instructions)
            {
                if (instruct.OP == Operation.Coerce &&
                    (++coereceCount == 2))
                {
                    var coerceIns = (CoerceIns)instruct;
                    coerceIns.TypeNameIndex = socketConnection.QNameIndex;
                    break;
                }
            }
            instructions.InsertRange(instructions.Count - 5, new Instruction[]
            {
                new GetLocal2Ins(),
                new PushIntIns(abc.Pool) { Value = 4001 },
                new GetLocalIns(6),
                new NewArrayIns(1),
                new CallPropVoidIns(abc.Pool) { PropertyNameIndex = sendMessageQNameIndex, ArgCount = 2 }
            });
        ...
        protected byte[] TrimSendCode(ABCFile abc, ASCode sendCode)
        {
            sendCode.Deobfuscate(DeobfuscationLevels.ControlFlow);

            var trimmed = sendCode.Instructions
                .SkipWhile(i => i.OP != Operation.IfNLt)
                .SkipWhile(i => i.OP != Operation.GetLocal_0)
                .ToArray();

            sendCode.Instructions.Clear();
            sendCode.Instructions.AddRange(trimmed);
            List<Instruction> instructions = sendCode.Instructions;
            for (int i = 0; i < instructions.Count; i++)
            {
                Instruction instruct = instructions[i];
                if (Local.IsValid(instruct.OP))
                {
                    var local = (Local)instruct;
                    switch (local.Register)
                    {
                        case 2:
                        instructions[i] = new GetLocal1Ins();
                        break;

                        case 3:
                        instructions[i] = new GetLocal2Ins();
                        break;

                        case 4:
                        {
                            if (Local.IsGetLocal(instruct.OP))
                            {
                                instructions[i] = new GetLocal3Ins();
                            }
                            else if (Local.IsSetLocal(instruct.OP))
                            {
                                instructions[i] = new SetLocal3Ins();
                            }
                            break;
                        }
                    }
                }
                else if (instruct.OP == Operation.Debug)
                {
                    var debugIns = (DebugIns)instruct;
                    if (debugIns.RegisterIndex == 4)
                    {
                        debugIns.RegisterIndex = 2;
                        debugIns.NameIndex = abc.Pool.AddString("data");
                    }
                }
            }
            instructions.InsertRange(0, new Instruction[]
            {
                new GetLocal0Ins(),
                new Instruction(Operation.PushScope),
                new DebugIns(abc.Pool, "header", 1, 0),
                new DebugIns(abc.Pool, "values", 1, 1)
            });

            return sendCode.ToArray();
        }
        protected ASCode DisableEncryption(ASMethod sendMethod)
        {
            ASCode sendCode = sendMethod.Body.ParseCode();
            sendCode.Deobfuscate(DeobfuscationLevels.ControlFlow);

            bool isComplete = false;
            for (int i = 0, neCount = 0; i < sendCode.Instructions.Count; i++)
            {
                ...
                switch (instruct.OP)
                {
                    ...
                    case Operation.CallPropVoid:
                    {
                        var callPropVoidIns = (CallPropVoidIns)instruct;
                        if (callPropVoidIns.PropertyName.Name == "writeBytes")
                        {
                            Instruction previousIns = sendCode.Instructions[i - 1];
                            if (!Local.IsGetLocal(previousIns.OP))
                            {
                                sendCode.Instructions.RemoveRange(i - 4, 2);
                                i -= 2;

                                sendCode.Instructions.RemoveAt(i - 1);
                                i -= 1;
